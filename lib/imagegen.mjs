...
function normalizeSize(size) {
  ...
  if (width > 3840 || height > 3840) {
    throw new Error("Maximum edge length is 3840px.");
  }

  if (width % 16 !== 0 || height % 16 !== 0) {
    throw new Error("Both image dimensions must be multiples of 16px.");
  }
  ...
  if (longEdge / shortEdge > 3) {
    throw new Error("The long edge to short edge ratio must not exceed 3:1.");
  }

  const totalPixels = width * height;

  if (totalPixels < 655360 || totalPixels > 8294400) {
    throw new Error("Total pixels must be between 655,360 and 8,294,400.");
  }
...
async function runImageRequest({
  ...
  return {
    ...
    sha256: crypto.createHash("sha256").update(base64Image).digest("hex")
  };
}
