...
const REQUIRED_USAGE_FIELDS = ["baseUrl", "apiKey"];
...
export class RuntimeConfigUsageError extends Error {
  constructor({ missingFields, configPath }) {
    super(formatUsageSetupMessage({ missingFields, configPath }));
    this.name = "RuntimeConfigUsageError";
...
function formatUsageSetupMessage({ missingFields, configPath }) {
  const labels = missingFields.map((field) => USAGE_FIELD_LABELS[field] || field).join(", ");

  return [
    "Configure the runtime settings before using the image tools.",
    `Missing required fields: ${labels}.`,
    `Save them in ${configPath}.`,
    "For guided setup, run `npx mcp-imagegen-server --configure`."
  ].join(" ");
}
...
export async function readRuntimeConfigFile({ env = process.env, configPath, platform = process.platform } = {}) {
  const resolvedConfigPath = configPath || resolveConfigPath({ env, platform });
...
export async function getRuntimeConfigState({ env = process.env, configPath, platform = process.platform } = {}) {
...
export async function getRuntimeConfigReadiness({ env = process.env, configPath, platform = process.platform } = {}) {
  const state = await getRuntimeConfigState({ env, configPath, platform });
  const missingFields = REQUIRED_USAGE_FIELDS.filter((field) => !state.effectiveConfig[field]);

  return {
    ...state,
    missingFields,
    readyForUsage: missingFields.length === 0
  };
}
...
export async function assertRuntimeConfigReady({ env = process.env, configPath, platform = process.platform } = {}) {
  const readiness = await getRuntimeConfigReadiness({ env, configPath, platform });

  if (!readiness.readyForUsage) {
    throw new RuntimeConfigUsageError({
...
export async function loadRuntimeConfig({ env = process.env, configPath, platform = process.platform } = {}) {
  const state = await assertRuntimeConfigReady({ env, configPath, platform });
